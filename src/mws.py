...
class Products(MWS):
    ...
    def get_my_fee_estimate(self, marketplaceid, sku, price, shipping):
        data = dict(Action='GetMyFeesEstimate')
        data.update(utils.enumerate_param('FeesEstimateRequestList.FeesEstimateRequest.1.Identifier.', str(sku)))
        data.update(utils.enumerate_param('FeesEstimateRequestList.FeesEstimateRequest.1.IdType.', 'SellerSKU'))
        data.update(utils.enumerate_param('FeesEstimateRequestList.FeesEstimateRequest.1.IdValue.', str(sku)))
        data.update(
            utils.enumerate_param('FeesEstimateRequestList.FeesEstimateRequest.1.MarketplaceId.', marketplaceid))
        data.update(utils.enumerate_param(
        ...
        data.update(utils.enumerate_param(
            ...
            'EUR'))
        data.update(utils.enumerate_param('FeesEstimateRequestList.FeesEstimateRequest.1.IsAmazonFulfilled.',
                                          'false'))
        data.update(
            utils.enumerate_param('FeesEstimateRequestList.FeesEstimateRequest.1.PriceToEstimateFees.Shipping.Amount.',
                                  str(shipping)))
        data.update(utils.enumerate_param(
            'FeesEstimateRequestList.FeesEstimateRequest.1.PriceToEstimateFees.Shipping.CurrencyCode', 'EUR'))
        return self.make_request(data)

    def get_my_fee_estimate_list(self, marketplace_id, items, shipping):
        data = dict(Action='GetMyFeesEstimate')
        i = 1
        for item in items:
            data.update(utils.enumerate_param('FeesEstimateRequestList.FeesEstimateRequest.' + str(i) + '.Identifier.',
                                              str(item['sku'])))
            data.update(utils.enumerate_param('FeesEstimateRequestList.FeesEstimateRequest.' + str(i) + '.IdType.',
                                              'SellerSKU'))
            data.update(utils.enumerate_param('FeesEstimateRequestList.FeesEstimateRequest.' + str(i) + '.IdValue.',
                                              str(item['sku'])))
            data.update(
                utils.enumerate_param('FeesEstimateRequestList.FeesEstimateRequest.' + str(i) + '.MarketplaceId.',
                                      marketplace_id))
            data.update(utils.enumerate_param(
                'FeesEstimateRequestList.FeesEstimateRequest.' + str(i) + '.PriceToEstimateFees.ListingPrice.Amount',
                str(item['price'])))
            data.update(utils.enumerate_param(
                'FeesEstimateRequestList.FeesEstimateRequest.' + str(i) +
                '.PriceToEstimateFees.ListingPrice.CurrencyCode', 'EUR'))
            data.update(
                utils.enumerate_param('FeesEstimateRequestList.FeesEstimateRequest.' + str(i) + '.IsAmazonFulfilled.',
                                      'false'))
            data.update(utils.enumerate_param('FeesEstimateRequestList.FeesEstimateRequest.' +
                                              str(i) + '.PriceToEstimateFees.Shipping.Amount.', str(shipping)))
            data.update(utils.enumerate_param(
                'FeesEstimateRequestList.FeesEstimateRequest.' + str(i) + '.PriceToEstimateFees.Shipping.CurrencyCode',
                'EUR'))
            i += 1
        return self.make_request(data)
